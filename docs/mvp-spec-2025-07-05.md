# MCP–JCR Kotlin Server (Read-Only MVP Specification)
...
**Stack:** Kotlin 2.0+ • Spring Boot 3.5+ • Gradle 8.10+ (Kotlin DSL) • Spring AI MCP WebFlux Starter • Apache Jackrabbit 2.23+ • JCR API • Docker/macOS build
...
├── Contents/
│   ├── MacOS/
│   │   └── mcp-jcr-manager (Tauri executable ~3MB)
│   ├── Resources/
│   │   └── mcp-jcr-server.jar (~50MB)
│   └── Info.plist
```
...
## 🌐 Remote Repository Architecture (Phase 2)

### Recommended Approach: Local MCP → Remote JCR

```
[Developer Machine]                    [Remote Server]
┌─────────────────┐                   ┌──────────────────┐
│ Claude/IDE      │                   │ CMS (Magnolia)   │
│       ↓         │                   │       ↓          │
│ MCP Server      │ ←─── HTTPS ────→ │ JCR Repository   │
│ (localhost:8181)│     WebDAV        │ (port 8080)      │
└─────────────────┘                   └──────────────────┘
```
...
- **Remote Connection Options**
  - **Option A: Local MCP → Remote JCR** (Recommended)
